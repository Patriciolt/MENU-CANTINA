...
// ✅ Después de mostrar todas las promos, mostramos una pantalla "Menú".
const MENU_SCREEN_MS = 12_000;
...
function setQRs(){
  ...
  const qrMenuBig = "https://api.qrserver.com/v1/create-qr-code/?size=760x760&data=" + encodeURIComponent(MENU_URL);
  ...
  $("qrMenu").src = qrMenu;
  const big = document.getElementById("menuQrBig");
  if(big) big.src = qrMenuBig;
  $("qrWapp").src = qrWapp;
  $("qrIg").src   = qrIg;
}
...
function startParallax(){
  ...
  function tick(now){
    const t = (now - t0) / 1000;
    if(wm){
      const x = 82 + Math.sin(t * 0.09) * 3.6;
      const y = 56 + Math.cos(t * 0.075) * 2.6;
      wm.style.backgroundPosition = `${x}% ${y}%`;
    }
    if(pt){
      const px = (Math.sin(t * 0.045) * 26);
      const py = (Math.cos(t * 0.04) * 20);
...
function startRotation(){
  ...
  const tick = () => {
    ...
    if(state === "promo"){
      ...
      // si fue la última promo, la próxima pantalla será el menú
    ...
    showMenuScreen();
    state = "promo";
    rotateTimer = setTimeout(tick, MENU_SCREEN_MS);
  };
