...
========================= */
function setQRs(){
  // QRs grandes (en slides). Se generan 1 vez y quedan cacheados por el navegador.
  const size = 520;
  const menuQR = `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(MENU_URL)}`;
  const wappQR = `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(WHATSAPP_URL)}`;
  const igQR = `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(INSTAGRAM_URL)}`;
...
/* -------------------------
   PANTALLAS CTA (15s)
   ------------------------- */
function dimPromo(on){
  const media = $("promoMedia");
  const info = $("promoInfo");
  if(media) media.style.opacity = on ? "0.18" : "1";
  if(info) info.style.opacity = on ? "0.18" : "1";
}
...
function showMenuSlide(){
  ...
  const big = $("qrMenuBig");
  if(big) big.src = `https://api.qrserver.com/v1/create-qr-code/?margin=8&size=700x700&data=${encodeURIComponent(MENU_URL)}`;
}
function hideMenuSlide(){ hideSlide("menuSlide"); }

function showWappSlide(){
  showSlide("wappSlide");
  const number = $("wappNumber");
  if(number) number.textContent = WAPP_NUMBER_E164.replace(/^\+/,'+');
  const big = $("qrWappBig");
  if(big) big.src = `https://api.qrserver.com/v1/create-qr-code/?margin=8&size=700x700&data=${encodeURIComponent(WHATSAPP_URL)}`;
}
function hideWappSlide(){ hideSlide("wappSlide"); }

function showIgSlide(){
  showSlide("igSlide");
  const handle = $("igHandle");
  if(handle){
    const clean = INSTAGRAM_URL.replace(/^https?:\/\//,'').replace(/^www\./,'');
    handle.textContent = clean;
  }
  const big = $("qrIgBig");
  if(big) big.src = `https://api.qrserver.com/v1/create-qr-code/?margin=8&size=700x700&data=${encodeURIComponent(INSTAGRAM_URL)}`;
}
function hideIgSlide(){ hideSlide("igSlide"); }

function hideAllCta(){
  // Oculta todas las pantallas CTA (Menú / WhatsApp / Instagram)
  // (bugfix: antes se llamaba recursivamente y rompía todo el render)
  hideMenuSlide();
  hideWappSlide();
  hideIgSlide();
}
...
function startRotation(){
  ...
  let i = 0;
  let mode = "promo"; // promo -> menu -> wapp -> ig -> promo
