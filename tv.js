...
async function cargarPromos() {
  ...
  const pickIndexContains = (candidates) => {
    const norm = (s) => String(s || '').trim().toLowerCase();
    const hs = headers.map(norm);
    for (const c of candidates) {
      const needle = norm(c);
      const i = hs.findIndex(h => h.includes(needle));
      if (i >= 0) return i;
    }
    return -1;
  };
  ...
  const iDesc    = pickIndexContains(["TV DESCRIPCION","TV_DESCRIPCION","DESCRIPCION TV","TV DESCRIPCIÓN","TV_DESCRIPCIÓN"]);
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const desc = (typeof iDesc !== "undefined" && iDesc >= 0) ? String(row[iDesc] || "").trim() : "";

    out.push({ orden, titulo, precio, imagen, desc });
  }
...
function renderPromo(p, total) {
  ...
  if (el.promoDesc) {
    const d = String(promo.desc || '').trim();
    el.promoDesc.textContent = d;
    el.promoDesc.style.display = d ? 'block' : 'none';
  }
