...
========================= */
function setQRs(){
  // ✅ Generamos QRs con Google Chart (más confiable en GitHub Pages / HTTPS)
  // Algunas redes bloquean api.qrserver.com, por eso cambiamos la fuente.
  const mk = (url) =>
    "https://chart.googleapis.com/chart?cht=qr&chs=280x280&chld=L|0&chl=" + encodeURIComponent(url);

  $("qrMenu").src = mk(MENU_URL);
  $("qrWapp").src = mk(WHATSAPP_URL);
  $("qrIg").src   = mk(INSTAGRAM_URL);
}
...
function startParallax(){
  ...
  function tick(now){
    const t = (now - t0) / 1000;
    if(wm){
      // Movimiento un poco mayor y más suave
      const x = 82 + Math.sin(t * 0.08) * 3.6;
      const y = 56 + Math.cos(t * 0.07) * 3.0;
      wm.style.backgroundPosition = `${x}% ${y}%`;
    }
    if(pt){
      const px = (Math.sin(t * 0.035) * 26);
      const py = (Math.cos(t * 0.032) * 26);
...
function renderPromo(p){
  ...
  // ❌ Sin porcentaje de descuento (pedido del usuario)
