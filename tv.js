...
function renderPromo(p){
  ...
  // Título: usamos SOLO TV TITULO (no mostramos el nombre original)
  const titulo = (p?.tvTitulo && String(p.tvTitulo).trim()) ? String(p.tvTitulo).trim() : "PROMO";
...
function showWappSlide(){
  ...
  const api = "https://api.qrserver.com/v1/create-qr-code/?color=000000&bgcolor=FFFFFF&margin=8&";
  const big = $("qrWappBig");
  if(big) big.src = api + "size=700x700&data=" + encodeURIComponent(WAPP_URL);

  // Texto
  const wtxt = $("wappWappTxt");
  if(wtxt) wtxt.textContent = `WhatsApp: ${WAPP_NUMBER_E164}`;
}

function hideWappSlide(){
  const slide = $("wappSlide");
  if(!slide) return;
  slide.classList.add("hidden");
  slide.setAttribute("aria-hidden","true");
...
function showIgSlide(){
  ...
  const api = "https://api.qrserver.com/v1/create-qr-code/?color=000000&bgcolor=FFFFFF&margin=8&";
  const big = $("qrIgBig");
  if(big) big.src = api + "size=700x700&data=" + encodeURIComponent(IG_URL);

  const itxt = $("igHandleTxt");
  if(itxt) itxt.textContent = IG_HANDLE;
}

function hideIgSlide(){
  const slide = $("igSlide");
  if(!slide) return;
  slide.classList.add("hidden");
  slide.setAttribute("aria-hidden","true");
...
function startRotation(){
  ...
  if(!promos.length){
    // Fallback: si no hay promos (o falló la carga), igualmente rotamos las pantallas de QR
    let modeQR = "menu";
    const stepQR = () => {
      renderPromo(null);
      if(modeQR === "menu"){
        showMenuSlide();
        hideWappSlide(); hideIgSlide();
        modeQR = "wapp";
      } else if(modeQR === "wapp"){
        hideMenuSlide();
        showWappSlide();
        ...
        hideMenuSlide(); hideWappSlide();
        showIgSlide();
        modeQR = "menu";
      }
      rotateTimer = setTimeout(stepQR, STEP_MS);
    };
    if(st) st.textContent = "";
    stepQR();
    return;
  ...
  const tick = () => {
    if(mode === "promo"){
      ...
      if(i >= promos.length){
        // Terminar ciclo de promos -> mostrar QR MENÚ
        mode = "menu";
        showMenuSlide();
      }else{
        hideMenuSlide();
    ...
    // mode === "ig" -> volver a empezar promos
    hideIgSlide();
    mode = "promo";
    i = 0;
    renderPromo(promos[i]);
