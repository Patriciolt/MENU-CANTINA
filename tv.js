function setQR(el, url){ if(!el) return; el.src = 'https://chart.googleapis.com/chart?cht=qr&chs=560x560&chld=L|0&chl=' + encodeURIComponent(url); }
/* =========================
   CONFIG
========================= */
...
function setQRs(){
  ...
  $("qrMenuBig").src = qrMenu;
  $("qrWappBig").src = qrWapp;
  $("qrIg").src   = qrIg;
}
...
function startRotation(){
  ...
  // helpers to show/hide QR slides
  const menuSlide = document.getElementById("menuSlide");
  const wappSlide = document.getElementById("wappSlide");
  const igSlide   = document.getElementById("igSlide");

  function hideSlides(){
    [menuSlide, wappSlide, igSlide].forEach(sl => {
      if(!sl) return;
      sl.classList.add("hidden");
      sl.setAttribute("aria-hidden","true");
    });
    document.body.classList.remove("is-qr-slide");
  }
  ...
  function showSlide(sl){
    ...
    sl.setAttribute("aria-hidden","false");
    document.body.classList.add("is-qr-slide");
  }

  const total = promos.length;
  let promoIndex = 0;

  function renderCurrentPromo(){
    hideSlides();
    const p = promos[promoIndex % total];
    renderPromo(p, total, promoIndex);
    promoIndex++;
  }

  // Progress bar (15s en todo: promos y QR)
  let t0 = performance.now();
  ...
  // ciclo: promos -> menu -> wapp -> ig -> vuelve a promos
  function runTick(){
    // si aún quedan promos del ciclo
    if(promoIndex < total){
      renderCurrentPromo();
      return;
    }

    // Después de la última promo, mostramos 3 QR (15s cada uno)
    const cyclePos = (promoIndex - total) % 3; // 0 menu, 1 wapp, 2 ig
    if(cyclePos === 0) showSlide(menuSlide);
    if(cyclePos === 1) showSlide(wappSlide);
    if(cyclePos === 2) showSlide(igSlide);

    promoIndex++;

    // Cuando terminamos los 3 QR, reiniciamos ciclo de promos
    if(promoIndex >= total + 3){
      promoIndex = 0;
    }
  }
